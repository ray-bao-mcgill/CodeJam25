...
const BaseQuestion: React.FC<BaseQuestionProps> = ({
  ...
  // Show waiting screen when waiting for others
  if (isWaitingForOthers) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 game-bg">
        ...
        >
          <div className="game-paper px-8 py-10 game-shadow-hard-lg">
            <div className="text-center space-y-6">
              <div className="text-4xl font-black mb-4">WAITING FOR OTHER PLAYERS...</div>
              <div className="text-lg text-gray-800">
                You've submitted your answer. Waiting for other players to finish...
              </div>
              <div className="flex items-center justify-center gap-4 mt-8">
                <div className="game-label-text text-sm">PLAYERS READY:</div>
                <div className="text-3xl font-black text-gray-800">
                  {submittedPlayersCount} / {totalPlayersCount}
                </div>
              </div>
              {totalPlayersCount > 0 && submittedPlayersCount < totalPlayersCount && (
                <div className="text-sm text-gray-600 mt-4">
                  Waiting for {totalPlayersCount - submittedPlayersCount} more player{totalPlayersCount - submittedPlayersCount === 1 ? '' : 's'}...
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-center gap-4 flex-wrap">
          ...
          <button
            ...
            style={{
              ...
              color: answer.trim() && !isSubmitting && !isLoading
                ? 'var(--game-text-white)'
                : 'var(--game-text-dim)',
              cursor: answer.trim() && !isSubmitting && !isLoading
                ? 'pointer'
                : 'not-allowed',
              opacity: (!answer.trim() || isSubmitting || isLoading) ? 0.5 : 1
            }}
          >
